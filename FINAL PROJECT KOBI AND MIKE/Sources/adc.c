...
void adc_init(void) {
	...
	enable_irq(INT_ADC0 - 16);
	set_irq_priority(INT_ADC0 - 16, 2); // Interrupt priority = 2 - less than TPM to allow measuring
}
...
int adc_cal(void) {
	...
	calib += ADC0_CLPS + ADC0_CLP4 + ADC0_CLP3 + ADC0_CLP2 + ADC0_CLP1
			+ ADC0_CLP0;
	calib /= 2;
	calib |= 0x8000; 	// Set MSB 
	ADC0_PG = calib;
	calib = 0;
	calib += ADC0_CLMS + ADC0_CLM4 + ADC0_CLM3 + ADC0_CLM2 + ADC0_CLM1
			+ ADC0_CLM0;
	calib /= 2;
	calib |= 0x8000;	// Set MSB
	ADC0_MG = calib;
